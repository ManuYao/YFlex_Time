...
export const getHowToItems = (timer) => {
  if (timer.id === 'amrap') {
    ...
    return [
      { icon: '🔄', lead: 'Circuit à enchaîner', text: `Fais un maximum de tours de ton circuit en ${duration} — le nombre de tours n'est pas limité.` },
      { icon: '🏃', lead: 'Cardio en continu', text: `Course, vélo, tapis... tiens ${duration} sans t'arrêter et vise la distance ou l'intensité.` },
    ];
  }
  if (timer.id === 'basic') {
    ...
    return [
      { icon: '💪', lead: 'Ton effort, ton rythme', text: `Fais ta série (ex : 6 à 12 tractions), puis appuie pour lancer ${rest} de repos.` },
      { icon: '🔁', lead: `${rounds}`, text: 'Répète pour le nombre de tours réglé ci-dessus — ajustable à tout moment.' },
    ];
  }
  if (timer.id === 'emom') {
    ...
    return [
      { icon: '⏱️', lead: `Toutes les ${interval}`, text: `Un nouvel effort démarre à chaque intervalle, pendant ${rounds}.` },
      { icon: '💥', lead: 'Finis vite, repose plus', text: "Termine ta série (ex : 10 pompes) avant la fin de l'intervalle : le temps qu'il reste est ton repos." },
    ];
  }
  if (timer.id === 'tabata') {
    ...
    return [
      { icon: '🔥', lead: 'Travail chronométré', text: `${work} d'effort non-stop (pompes, squats...), puis ${rest} de repos.` },
      { icon: '🔄', lead: `${rounds} automatiques`, text: 'Le cycle travail/repos se répète tout seul, rien à relancer.' },
      { icon: '🔗', lead: "Change d'exercice à chaque tour", text: 'Tractions, tirage à la poulie, pompes... enchaîne comme tu veux, le but est de bosser le muscle à ta façon.' },
    ];
  }
  if (timer.id === 'mix') {
    return [
      { icon: '🔀', lead: 'Ton enchaînement', text: 'Combine plusieurs blocs (AMRAP, EMOM, TABATA...) que tu configures toi-même dans le constructeur.' },
      { icon: '🧩', lead: '100% modulable', text: 'Ajoute, réordonne ou supprime des blocs pour coller exactement à ta séance.' },
    ];
  }
  return [{ icon: '🏅', lead: timer.name, text: timer.full }];
};
