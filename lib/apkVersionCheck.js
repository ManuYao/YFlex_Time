 ...
 *
 * Le blocage est déclenché par UNE seule chose, point final : la version
 * installée est réellement plus ancienne que `min_apk_version`. Rien
 * d'autre à changer dans le Gist pour bloquer les anciennes versions
 * (décision utilisateur du 18/09/2026 — le champ `is_forced_update` a été
 * essayé comme coupe-circuit optionnel puis abandonné pour rester simple :
 * une seule chose à retenir, pas de deuxième condition qui peut être
 * oubliée dans le Gist et empêcher le blocage sans qu'on comprenne
 * pourquoi).
 *
...
const toState = (payload, extra = {}) => {
  ...
  return emptyState({
    checked: true,
    isBlockedByForcedUpdate: outdated,
