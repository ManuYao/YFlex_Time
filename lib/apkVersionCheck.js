 ...
 *
 * Le blocage est déclenché par UNE seule chose : la version installée est
 * réellement plus ancienne que `min_apk_version`. Bumper ce champ suffit —
 * pas de second interrupteur à penser en même temps (décision utilisateur du
 * 18/09/2026, après confusion sur l'ancien système à deux conditions).
 *
 * `is_forced_update` devient un coupe-circuit EXPLICITE, pas un déclencheur :
 * le mettre à `false` désactive le blocage même si la version est dépassée
 * (utile pour annoncer un futur `min_apk_version` sans bloquer tout de
 * suite). Absent ou `true` → comportement par défaut, on bloque dès que la
 * version ne correspond pas.
 *
...
const toState = (payload, extra = {}) => {
  ...
  return emptyState({
    checked: true,
    isBlockedByForcedUpdate: outdated && payload?.is_forced_update !== false,
