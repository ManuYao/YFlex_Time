...
const blockDuration = (block) => {
  ...
  if (block.type === 'basic') {
    return block.duration * rounds + (block.rest || 0) * Math.max(0, rounds - 1);
  }
...
export const computeSessionStats = (timer, elapsedSeconds) => {
  ...
  if (timer.id === 'basic') {
    // Pattern: W R W R ... W (rest between rounds, ends on last work)
    let workTotal = 0;
    let restTotal = 0;
    let cursor = 0;
    let completed = 0;
    for (let r = 1; r <= rounds; r++) {
      const wEnd = cursor + work;
      if (elapsedSeconds >= wEnd) {
        workTotal += work;
        completed = r;
      } else {
      ...
      cursor = wEnd;
      if (r < rounds && rest > 0) {
        const rEnd = cursor + rest;
        if (elapsedSeconds >= rEnd) {
          restTotal += rest;
        } else {
          restTotal += Math.max(0, elapsedSeconds - cursor);
          break;
        }
        cursor = rEnd;
      }
    ...
    for (const b of blocks) {
      ...
      if (b.type === 'rest') {
        ...
        for (let r = 1; r <= b.rounds; r++) {
          const wEnd = local + b.duration;
          if (inThisBlock >= wEnd) workTotal += b.duration;
          else { workTotal += Math.max(0, inThisBlock - local); break; }
          local = wEnd;
          if (r < b.rounds && (b.rest || 0) > 0) {
            const rEnd = local + b.rest;
            if (inThisBlock >= rEnd) restTotal += b.rest;
            else { restTotal += Math.max(0, inThisBlock - local); break; }
            local = rEnd;
          }
        }
      } else {
