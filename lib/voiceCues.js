...
export const decideCues = (prev, cur, { mode } = {}) => {
  ...
  // phase est traité et renvoyé plus haut, donc à ce stade prev et cur
  // portent forcément sur la MÊME phase (même phaseTotal). Exclu pour le
  // mode AMRAP pur : sa seule "phase" est déjà toute la séance, ce serait
  // le même repère compté deux fois.
  const isAmrapMode = mode === 'amrap';
  if (!isAmrapMode && cur.phaseTotal >= PHASE_CUE_MIN_TOTAL) {
    const prevPhaseProgress = 1 - prev.phaseLeft / prev.phaseTotal;
    const curPhaseProgress = 1 - cur.phaseLeft / cur.phaseTotal;
    if (crossed(prevPhaseProgress, curPhaseProgress, 0.5) && !cues.includes('half')) {
      cues.push('half');
    }
    if (prev.phaseLeft > 10 && cur.phaseLeft <= 10 && cur.phaseLeft > 3 && !cues.includes('last_10s')) {
      cues.push('last_10s');
    }
  }
  ...
  // En repos, les mêmes repères ne disent pas la même chose : « dix
  // secondes, tout donner ! » n'a pas de sens quand on souffle — il faut
  // « dix secondes, prépare-toi ». Variantes dédiées dans lib/voiceTexts.js.
  if (cur.isRest) {
    return cues.map((c) => (REST_VARIANTS.has(c) ? `${c}_rest` : c));
  }
  return cues;
};

const REST_VARIANTS = new Set(['half', 'almost', 'last_10s']);
