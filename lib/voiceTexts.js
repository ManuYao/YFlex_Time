...
// ═════════════════════════════════════════════════════════════════════════
// Style PAR DÉFAUT. Au lieu des phrases motivantes ci-dessus (style « Motivant », qui reste
// au choix et n'est jamais supprimé), le coach donne juste
// l'info, tranchée, en une ou deux secondes, comme on annonce une série :
// « 3 sur 8. », « Repos. 30 secondes. », « 2 minutes. », « 10 secondes. ».
...
// « 3 sur 8. » — dans un MIX, seulement au changement de bloc
...
const roundInfo = ({ prev, cur, mode }) => {
  ...
  return `${cur.currentRound} sur ${cur.totalRounds}.`;
};
...
export const conciseText = (situation, ctx) => {
  const { cur } = ctx;
  if (situation.startsWith('remaining_rest_')) return null; // « X sur Y » le dira au prochain effort
  ...
  switch (situation) {
    ...
    // AMRAP : une seule phase qui EST la séance, le temps annoncé est clair.
    case 'half':
      return `Moitié. ${spokenDuration(cur.totalLeft)}.`;
    case 'almost':
      return `${spokenDuration(cur.totalLeft)}.`;
    // Autres modes : un temps de SÉANCE annoncé seul se confondrait avec le
    // temps de la phase en cours (« 23 secondes » en plein repos de 10 s) —
    // on dit donc de quoi il s'agit.
    case 'session_half':
      return 'Mi-séance.';
    case 'session_almost':
    ...
    case 'emom_half':
    case 'boost':
      return `${spokenDuration(cur.phaseLeft)}.`;
    case 'last_10s':
    case 'rest_last_10s':
    case 'emom_last_10s':
