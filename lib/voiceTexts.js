...
// ═══ MIX : changement d'exercice ═══
// {name} = le nom du bloc tel que tu l'as écrit dans le Mix Builder
// (« Pompes », « Échauffement »…), ou son type s'il n'a pas de nom.
export const BLOCK_TEMPLATES = {
  // Tout premier bloc, juste après le GO.
  first_block: [
    'On commence par {name} !',
    "C'est parti avec {name} !",
  ],
  block_start: [
    'Place à {name} !',
    'Bloc suivant : {name}, on y va !',
    '{name}, à toi de jouer !',
  ],
  last_block_start: [
    'Dernier bloc : {name}, tout ce qui te reste !',
    'Dernier bloc, {name} ! On lâche rien !',
  ],
  // Dix secondes avant la fin d'un bloc, pour se préparer.
  next_block: [
    'Prochain bloc : {name}, prépare-toi.',
    'Ensuite, {name}. Prépare-toi !',
  ],
  // … quand le bloc suivant est une pause.
  next_rest: [
    'Ensuite, une pause bien méritée.',
    'Tiens bon, une pause arrive.',
  ],
};
...
// Nom d'un bloc de MIX à voix haute : le nom écrit dans le Mix Builder, ou
// son type s'il n'en a pas. Un nom tout en majuscules (« BURPEES ») risque
// d'être épelé lettre par lettre par la synthèse vocale : on le remet en
// minuscules (sauf les sigles courts).
const TYPE_NAMES = { amrap: 'Amrap', emom: 'Émom', tabata: 'Tabata', rest: 'Repos' };
export const spokenBlockName = (label, type) => {
  const clean = (label || '').trim();
  if (!clean) return TYPE_NAMES[type] || 'Bloc';
...
// « Tour 3 sur 8. » — dans un MIX, ce sont les tours À L'INTÉRIEUR du bloc.
const roundInfo = ({ cur, mode }) => {
  const round = mode === 'mix' ? cur.subRound : cur.currentRound;
  const total = mode === 'mix' ? cur.subTotalRounds : cur.totalRounds;
  if (!round || !total || total < 2) return null;
  return `Tour ${round} sur ${total}.`;
};

// Ce que contient un bloc : ses tours (TABATA, EMOM) ou sa durée.
const blockDetail = (cur) =>
  cur.subTotalRounds > 1 && cur.blockType !== 'amrap' && cur.blockType !== 'rest'
    ? `${cur.subTotalRounds} tours`
    : spokenDuration(cur.blockTotal);
...
export const conciseText = (situation, ctx) => {
  ...
  switch (situation) {
    ...
    case 'last_round':
      return 'Dernier tour.';
    case 'rest':
      return cur.phaseTotal > 0 ? `Repos. ${spokenDuration(cur.phaseTotal)}.` : 'Repos.';

    // ── MIX : changement d'exercice ──
    case 'block_start':
      return `Bloc ${cur.currentRound} sur ${cur.totalRounds} : ${spokenBlockName(cur.blockLabel, cur.blockType)}. ${blockDetail(cur)}.`;
    case 'last_block_start':
      return `Dernier bloc : ${spokenBlockName(cur.blockLabel, cur.blockType)}. ${blockDetail(cur)}.`;
    case 'next_block':
      return cur.nextBlock
        ? `Ensuite : ${spokenBlockName(cur.nextBlock.label, cur.nextBlock.type)}.`
        : null;
...
// Phrase « Motivant » d'un repère : les phrases au hasard ci-dessus, avec
// le nom de l'exercice pour les annonces de MIX.
export const motivatingText = (situation, ctx = {}) => {
  const { cur } = ctx;
  const fill = (key, list, name) => {
    const t = pickFrom(key, list, Math.random);
  ...
  switch (situation) {
    ...
    case 'last_block_start': {
      if (!cur) return null;
      // Un bloc de repos se présente comme un repos.
      if (cur.blockType === 'rest') return pickText('rest');
      const name = spokenBlockName(cur.blockLabel, cur.blockType);
      if (situation === 'last_block_start') {
        return fill('last_block_start', BLOCK_TEMPLATES.last_block_start, name);
      }
      return cur.currentRound === 1
        ? fill('first_block', BLOCK_TEMPLATES.first_block, name)
        : fill('block_start', BLOCK_TEMPLATES.block_start, name);
    }
    case 'next_block': {
      const next = cur?.nextBlock;
      if (!next) return null;
      if (next.type === 'rest') return fill('next_rest', BLOCK_TEMPLATES.next_rest, '');
      return fill('next_block', BLOCK_TEMPLATES.next_block, spokenBlockName(next.label, next.type));
    }
    default:
      return pickText(situation);
  }
};
